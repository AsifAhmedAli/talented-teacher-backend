...
const new_chat_room = (req, res) => {
  ...
  const insertChatroomQuery =
    "INSERT INTO chatrooms (admin_id, room_name) VALUES (?, ?)";
  conn.query(
    insertChatroomQuery,
    [admin_id, room_name],
    (err, chatroomResult) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to create the chat room" });
      }
      ...
      if (allVoters) {
        ...
        conn.query(fetchAllVotersQuery, (err, allVotersResult) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to fetch all teachers" });
          }
          ...
          const insertQuery =
            "INSERT INTO chatroomsmembers (chatroomID, memberID) VALUES ?";
          conn.query(insertQuery, [values], (err, result) => {
            if (err) {
              console.error(err);
              return res
                .status(500)
                .json({ error: "Failed to add members to the chat room" });
            }

            res
              .status(201)
              .json({ message: "Chat room and members added successfully" });
          });
        });
        ...
        const fetchExistingMembersQuery =
          "SELECT memberID FROM chatroomsmembers WHERE chatroomID = ?";
        conn.query(
          fetchExistingMembersQuery,
          [chatroomID],
          (err, existingMembers) => {
            if (err) {
              console.error(err);
              return res
                .status(500)
                .json({ error: "Failed to fetch existing members" });
            }
            ...
            const newVoterIDs = selectedVoters.filter(
              (voterID) => !existingMemberIDs.includes(voterID)
            );

            if (newVoterIDs.length === 0) {
              return res.status(400).json({
                error:
                  "All selected voters are already members of the chatroom",
              });
            }
        ...
        return res
          .status(400)
          .json({ error: "No voters selected for the chatroom" });
      }
    }
  );
};
...
const add_members = (req, res) => {
  ...
  // Fetch existing members of the chatroom
  const fetchExistingMembersQuery =
    "SELECT cm.memberID, t.name FROM chatroomsmembers cm INNER JOIN teachers t ON cm.memberID = t.id WHERE chatroomID = ?";
  conn.query(
    fetchExistingMembersQuery,
    [chatroomID],
    (err, existingMembers) => {
      ...
      const duplicateMembers = existingMembers.filter((member) =>
        teacherIDs.includes(member.memberID)
      );

      if (duplicateMembers.length > 0) {
        const existingMembersInfo = duplicateMembers.map((member) => ({
          id: member.memberID,
          name: member.name,
        }));
        return res.status(400).json({
          error: "Some teachers are already members of the chatroom",
          existingMembers: existingMembersInfo,
        });
      }

      // Filter out teacherIDs that are already associated with the chatroom
      const newTeacherIDs = teacherIDs.filter(
        (teacherID) => !existingMemberIDs.includes(teacherID)
      );

      if (newTeacherIDs.length === 0) {
        return res.status(400).json({
          error: "All selected teachers are already members of the chatroom",
        });
      }
      ...
      conn.query(fetchTeachersQuery, [newTeacherIDs], (err, results) => {
        ...
        if (results.length !== newTeacherIDs.length) {
          return res
            .status(404)
            .json({ error: "One or more teachers not found" });
        }
...
const message_attachments = (req, res) => {
  ...
  Promise.all(uploadPromises)
    .then((urls) => {
      ...
      // Insert the message into the 'messages' table
      const insertMessageQuery =
        "INSERT INTO messages (sender_id, chatroomID, message, timestamp) VALUES (?, ?, ?, ?)";
      conn.query(
        insertMessageQuery,
        [sender_id, chatroomID, message, timestamp],
        (err, messageResult) => {
          if (err) {
            console.error(err);
            return res
              .status(500)
              .json({ error: "Failed to save the message to the database" });
          }
          ...
          const insertAttachmentsQuery =
            "INSERT INTO attachments (chatroomID, sender_id, message_id, file_name, file_path, created_at) VALUES ?";
          conn.query(
            insertAttachmentsQuery,
            [attachments.map((att) => Object.values(att))],
            (err) => {
              if (err) {
                console.error(err);
                return res.status(500).json({
                  error: "Failed to save attachments to the database",
                });
              }

              res.status(200).json({
                success: true,
                message: "Attachments uploaded and saved successfully",
                urls,
              });
            }
          );
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ error: "Failed to upload attachments to Cloudinary" });
    });
};
...
const get_all_messages_of_a_chatroom1 = (req, res) => {
  const chatroomID = req.params.chatroomID; // Assuming you are passing the chatroomID in the URL parameter
...
const get_members_of_chat_room = (req, res) => {
  ...
  conn.query(getMembersQuery, [chatroomID], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch chat room members" });
    }

    if (result.length === 0) {
      return res
        .status(404)
        .json({ error: "No members found for the given chatroom ID" });
    }
...
const get_all_chat_rooms_of_one_user = (req, res) => {
  const user_id = req.params.id;
  // Retrieve all chat rooms from the 'chatrooms' table
  const getAllChatRoomsQuery =
    "SELECT ct.Room_name, ct.Id FROM chatrooms ct join chatroomsmembers ctm on ctm.ChatroomID = ct.Id where ctm.MemberID = ?";
  conn.query(getAllChatRoomsQuery, [user_id], (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Failed to fetch chat rooms" });
    }
